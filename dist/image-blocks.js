...
function buildImageBlocks(state, resolveSrc) {
    const ranges = [];
    ...
    tree.iterate({
        enter: (node) => {
            ...
            if (!match) {
                console.log('[AtomicEditor] Image regex did not match');
                return;
            }
            const [, alt, src] = match;
            if (!src)
 ...
 * used unchanged.
 */
export function imageBlocks(resolveSrc) {
    const resolve = resolveSrc ?? ((s) => s);
...
//# sourceMappingURL=image-blocks.js.map